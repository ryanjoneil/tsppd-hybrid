...
FocacciTSPPDPrecedeSetPropagator::FocacciTSPPDPrecedeSetPropagator(
    Home home,
...
FocacciTSPPDPrecedeSetPropagator::FocacciTSPPDPrecedeSetPropagator(Space& home, FocacciTSPPDPrecedeSetPropagator& p) :
    Propagator(home, p),
    next(p.next),
    pred(p.pred),
    succ(p.succ),
...
Propagator* FocacciTSPPDPrecedeSetPropagator::copy(Space& home) {
    return new (home) FocacciTSPPDPrecedeSetPropagator(home, *this);
}

size_t FocacciTSPPDPrecedeSetPropagator::dispose(Space& home) {
    next[index].cancel(home, *this, Int::PC_INT_VAL);
    (void) Propagator::dispose(home);
    return sizeof(*this);
}

PropCost FocacciTSPPDPrecedeSetPropagator::cost(const Space& home, const ModEventDelta& med) const {
    return PropCost::crazy(PropCost::LO, (unsigned int) next.size());
}

void FocacciTSPPDPrecedeSetPropagator::reschedule(Space& home) {
    next[index].reschedule(home, *this, Int::PC_INT_VAL);
}

ExecStatus FocacciTSPPDPrecedeSetPropagator::propagate(Space& home, const ModEventDelta& med) {
...
ExecStatus FocacciTSPPDPrecedeSetPropagator::post(
    Home home,
    ViewArray<Int::IntView>& next,
    ViewArray<Set::SetView>& pred,
    ViewArray<Set::SetView>& succ,
    ...
    (void) new (home) FocacciTSPPDPrecedeSetPropagator(home, next, pred, succ, index, problem);
    return ES_OK;
}

ExecStatus FocacciTSPPDPrecedeSetPropagator::precede(Space& home, const unsigned int i, const unsigned int j) {
...
ExecStatus FocacciTSPPDPrecedeSetPropagator::before(Space& home, const unsigned int i, const unsigned int j) {
    if (i == j)
        return ES_OK;
...
ExecStatus FocacciTSPPDPrecedeSetPropagator::after(Space& home, const unsigned int i, const unsigned int j) {
...
void TSPPD::Solver::tsppd_precede_set(Home home, IntVarArray& next, const TSPPDProblem& problem) {
    ...
    for (unsigned int index = 0; index < problem.nodes.size(); ++index) {
        ...
        GECODE_ES_FAIL(FocacciTSPPDPrecedeSetPropagator::post(home, next_view, pred_view, succ_view, index, problem));
    }
}