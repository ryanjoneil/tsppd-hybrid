...
GurobiSubtourEliminationCallback::GurobiSubtourEliminationCallback(
    const string sec_type_string,
    const TSPPDProblem& problem,
    map<pair<unsigned int, unsigned int>, GRBVar> arcs) :
    GurobiTSPCallback(problem, arcs), sec_type(parse_sec_type(sec_type_string)), all_nodes() {

    for (unsigned int node = 0; node < problem.nodes.size(); ++node)
        all_nodes.insert(node);
}

SECType GurobiSubtourEliminationCallback::parse_sec_type(string sec_type_string) {
...
void GurobiSubtourEliminationCallback::callback(GRBCallback* model, const vector<vector<unsigned int>>& subtours) {
    if (subtours.size() <= 1)
        return;
...
void GurobiSubtourEliminationCallback::cut_tour(GRBCallback* model, const vector<unsigned int>& tour) {
...
void GurobiSubtourEliminationCallback::cut_tour_cutset(GRBCallback* model, const vector<unsigned int>& tour) {
...
void GurobiSubtourEliminationCallback::cut_tour_subtour(GRBCallback* model, const vector<unsigned int>& tour) {
    GurobiTSPCallbackHandler* handler = static_cast<GurobiTSPCallbackHandler*>(model);
...
void GurobiSubtourEliminationCallback::cut_tour_hybrid(GRBCallback* model, const vector<unsigned int>& tour) {
