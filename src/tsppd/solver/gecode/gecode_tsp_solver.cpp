...
GecodeTSPSolver::GecodeTSPSolver(
...
TSPPDSolution GecodeTSPSolver::solve() {
    ...
    if (gist) {
        Gist::Options o;
        Gist::Print<GecodeTSPSpace> p("next");
        o.inspect.click(&p);
    ...
    unique_ptr<Base<GecodeTSPSpace>> engine;
    if (search_engine == SEARCH_DFS)
        engine = make_unique<DFS<GecodeTSPSpace>>(space.get(), o);
    else if (search_engine == SEARCH_LDS)
        engine = make_unique<LDS<GecodeTSPSpace>>(space.get(), o);
    else
        engine = make_unique<BAB<GecodeTSPSpace>>(space.get(), o);

    bool has_solution_limit = (solution_limit > 0);
    while (auto s = unique_ptr<GecodeTSPSpace>(engine->next())) {
...
void GecodeTSPSolver::initialize_tsp_options() {
...
void GecodeTSPSolver::initialize_option_brancher() {
...
void GecodeTSPSolver::initialize_option_discrepancy_limit() {
    discrepancy_limit = 0;
...
void GecodeTSPSolver::initialize_option_dual_bound() {
    dual_type = DUAL_NONE;
...
void GecodeTSPSolver::initialize_option_gist() {
    gist = options.find("gist") != options.end();
}

void GecodeTSPSolver::initialize_option_search() {
...
void GecodeTSPSolver::initialize_option_threads() {
    threads = 1;
...
shared_ptr<GecodeTSPSpace> GecodeTSPSolver::build_space() {
    return make_shared<GecodeTSPSpace>(problem);
}
